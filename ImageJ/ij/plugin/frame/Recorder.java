	...
	public static void record(String method, int a1, int a2, int a3, int a4) {
		...
		if (scriptMode&&method.startsWith("make")) {
			if (method.equals("makeRectangle"))
			{
				...
				setClipbordContents(a1+"\t"+a2+"\t"+a3+"\t"+a4);
				System.out.println("test!");
			  // Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
			   //Clipborad cp = new Clipboard("Roi Clip"); 
			 //  Transferable trandata = new StringSelection("test kevin");
			 //  StringSelection selection = new StringSelection("test kevin");
			   //clipboard.setContent(trandata, null);
			   //JOptionPane.showInternalMessageDialog(null, "copy to clipboard"+"a1 "+"a2 "+"a3 "+"a4","information", JOptionPane.INFORMATION_MESSAGE);
			// JOptionPane.showMessageDialog(null, "copy to clipboard"+"a1 "+"a2 "+"a3 "+"a4");
			}
			else if (method.equals("makeOval"))
				recordString("imp.setRoi(new OvalRoi("+a1+","+a2+","+a3+","+a4+"));\n");
			else if (method.equals("makeLine"))
			{
				//System.out.println("makeline");s
